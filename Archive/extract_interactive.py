...
def process_directory(directory_path, patterns, labels=None, output_json=False):
    """
    ...
    for root, _, files in os.walk(directory_path):
        for file in files:
            ...
            file_results = process_file(file_path, patterns, labels, output_json)
            if file_results:
...
def interactive_shell():
    """
    Starts an interactive chatbot-style shell for PII extraction.
    Users can input file paths, directory paths, URLs, or commands.
    """
    session = PromptSession(history=InMemoryHistory())
    console.print(Panel("🔍 [bold green]PII Extraction Interactive Shell[/bold green]\nType 'exit' to quit.", expand=False))
    ...
    while True:
        try:
            user_input = session.prompt("🔎 Enter path or URL > ", completer=completer).strip()

            if user_input.lower() in ["exit", "quit"]:
                console.print("[bold red]Exiting shell.[/bold red]")
                break
            elif user_input.lower() == "help":
                console.print("[bold cyan]Enter a file path, directory, or URL to extract PII.[/bold cyan]")
            ...
            if is_url(user_input):
                console.print(f"🌐 [bold cyan]Processing URL:[/bold cyan] {user_input}")
                extracted_data = process_url(user_input, PATTERNS)
            elif os.path.isdir(user_input):
                console.print(f"📂 [bold blue]Processing Directory:[/bold blue] {user_input}")
                extracted_data = process_directory(user_input, PATTERNS)
            elif os.path.isfile(user_input):
                console.print(f"📄 [bold green]Processing File:[/bold green] {user_input}")
                extracted_data = process_file(user_input, PATTERNS)
            else:
                console.print(f"[bold red]Error: '{user_input}' is not a valid file, directory, or URL.[/bold red]")
                continue

            if extracted_data:
                table = Table(title="🔍 Extracted PII Data")
                table.add_column("Category", style="bold cyan")
                table.add_column("Matches", style="bold yellow")

                for label, matches in extracted_data.items():
                    table.add_row(label, ", ".join(matches) if matches else "None")
...
def main():
    """
    Main function to handle command-line and interactive modes.
    """
    parser = argparse.ArgumentParser(description="Extract patterns from files, directories, or URLs.")
    parser.add_argument("path", type=str, nargs="?", default=None, help="File, directory, or URL to process (optional)")
    ...
    if args.path:
        ...
        if is_url(path):
            console.print(f"🌐 Processing URL: {path}")
            process_url(path, PATTERNS, args.labels, args.json)
        elif os.path.isdir(path):
            console.print(f"📂 Processing Directory: {path}")
            process_directory(path, PATTERNS, args.labels, args.json)
        elif os.path.isfile(path):
            console.print(f"📄 Processing File: {path}")
            process_file(path, PATTERNS, args.labels, args.json)
        else:
            console.print(f"[bold red]Error: '{args.path}' is not a valid file, directory, or URL.[/bold red]")
    else:
        interactive_shell()
